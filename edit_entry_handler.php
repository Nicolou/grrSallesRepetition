 ...
 * @package   root
 * @version   $Id: edit_entry_handler.php,v 1.9 2009-09-29 18:02:56 grr Exp $
 * @filesource
 *
...
// On affecte le beneficiare à la breve descitpion (name)  /ajuter par nico
    ...
    )
{
    ...
    foreach ( $_GET['rooms'] as $room_id ) {
        ...
        if ($rep_type != 0 && !empty($reps))  {
            if(UserRoomMaxBooking(getUserName(), $room_id, count($reps)-1+$compt) == 0) {
               showAccessDeniedMaxBookings($day, $month, $year, '', $room_id, $back);
              exit();
            }
        } else {
            if(UserRoomMaxBooking(getUserName(), $room_id, $compt) == 0) {
               showAccessDeniedMaxBookings($day, $month, $year, '', $room_id, $back);
              exit();
            }
        }
