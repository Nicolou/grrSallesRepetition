...
<table width="400" border="3" cellspacing="4">
	<tbody><b>Liste des utilisateurs</b></tbody>
	<tr>
		<td>Nom du groupe
		</td>
		<td>Forfait restant
	...
	<tr>
		...
		if ($r[1] < 0) {
			echo '<td style="color:white; background-color:red">' . $r[1] . '</td>';
		}
		else
		{
			echo '<td>' . $r[1] . '</td>';
...
echo '<td>' . $r[2] . '
		...
		<td>';
// formulaire pour envoi de l'utilisateur selectionner where la frame reportAdminUser.php 
echo '<form method="post" action="reportAdminUser.php" target="adminUser">		
		<INPUT TYPE="hidden" name="zicos" VALUE="'. $r[0] . '" />
		<INPUT TYPE="submit" VALUE="->" title="voir le détail de ' . $r[0] . ' dans la partie de droite" />
		</form>
		</td>
	</tr>
	';
