 ...
 * Dernière modification : $Date: 2009-09-29 18:03:57 $
 * @author    Laurent Delineau <laurent.delineau@ac-poitiers.fr>
 * @author    Gilles Martin
 * @copyright Copyright 2003-2009 Laurent Delineau
 * @link      http://www.gnu.org/licenses/licenses.html
 * @package   root
 * @version   $Id: admin_config_imap.php,v 1.1 2009-09-29 18:03:57 grr Exp $
 * @filesource
 *
...
if (isset($_POST['imap_statut'])) {
    if ($_POST['imap_statut'] == "no_imap") {
        $req = grr_sql_query("delete from grr_setting where NAME = 'imap_statut'");
...
if($etape==1){
  ...
  echo encode_message_utf8("<h3>Numero de port</h3>* IMAP, port par défaut : 143<br />* IMAP sécurisé, port par défaut 993<br />* POP3, port par défaut 110<br />* POP3 sécurisé, port par défaut
  <br/>");
  ...
  if ($imap_type=="/imap") echo "selected=\"selected\"";
	echo ">IMAP</option>";
	echo "<option value=\"/pop3\"";
  if ($imap_type=="/pop3") echo "selected=\"selected\"";
	echo ">POP</option>";
  echo "</select>";
  ...
  echo "<select name=\"server_ssl\">";
	echo " <option value=\"\" ";
  if ($imap_ssl=="") echo "selected=\"selected\"";
	echo ">Ne pas utiliser SSL</option>";
  echo "<option value=\"/ssl\"";
	if ($imap_ssl=="/ssl") echo "selected=\"selected\"";
  echo ">Utiliser SSL</option>";
	echo "</select>";
  echo "<select name=\"server_cert\" >";
	echo "<option value=\"\" ";
	if ($imap_cert=="") echo "selected=\"selected\"";
	echo ">(non précisé)</option>";
	echo "<option value=\"/novalidate-cert\" ";
	if ($imap_cert=="/novalidate-cert") echo "selected=\"selected\"";
  echo ">NO-VALIDATE-CERT</option>";
	echo "<option value=\"/validate-cert\" ";
  if ($imap_cert=="/validate-cert") echo "selected=\"selected\"";
	echo ">VALIDATE-CERT</option>";
  echo "</select>";
  echo "<select name=\"server_tls\">";
	echo "<option value=\"\" ";
  if ($imap_tls=="") echo "selected=\"selected\"";
	echo ">(non précisé)</option>";
  echo "<option value=\"/tls\" ";
	if ($imap_tls=="/tls") echo "selected=\"selected\"";
  echo ">TLS</option>";
	echo "<option value=\"/notls\" ";
	if ($imap_tls=="/notls") echo "selected=\"selected\"";
  echo ">NO-TLS</option>";
	echo "</select>";
  ...
  if (!(function_exists("imap_open"))) {
    echo encode_message_utf8("<p class=\"avertissement\"><b>Attention </b> : les fonctions liées à l'authentification <b>IMAP/POP</b> ne sont pas activées sur votre serveur PHP.
    <br />La configuration IMAP/POP est donc actuellement impossible.</p></td></tr></table></body></html>");
    die();
  }
  if (getSettingValue("imap_statut") != '') {
    ...
    echo "<input type=\"hidden\" name=\"valid\" value=\"$valid\" />";
	  ...
	  if (getSettingValue("imap_statut")=='visiteur') echo "checked=\"checked\" ";
	  echo "/>Visiteur<br />";
    echo "<input type=\"radio\" name=\"imap_statut\" value=\"utilisateur\" ";
    if (getSettingValue("imap_statut")=='utilisateur') echo "checked=\"checked\" ";
    echo "/>Usager<br />";
    echo "Ou bien <br />";
    ...
    echo encode_message_utf8("<b>L'authentification IMAP/POP est donc pour le moment impossible</b>. Activez l'authentification IMAP/POP en choisissant le statut qui sera attribué aux personnes présentes dans l'annuaire IMAP/POP lorsqu'elles se connectent pour la première fois.
