    ...
    if ($room !=-1) {
        ...
        if ($test != "0") {
            ...
            if ($reg_admin_login != '') {
                ...
                if (grr_sql_command($sql) < 0) {fatal_error(0, "<p>" . grr_sql_error());}  else {$msg=get_vocab("add_user_succeed");}
            }
        }
    } else {
        // Domaine
        ...
        if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
        {
            ...
            if ($nb==0) {
                ...
                if (grr_sql_command($sql3) < 0) {fatal_error(0, "<p>" . grr_sql_error());}  else {$msg=get_vocab("add_user_succeed");}
            }
        }
    }
}
...
if ($action) {
    if ($action == "del_admin") {
        ...
        if (grr_sql_command($sql) < 0) {fatal_error(0, "<p>" . grr_sql_error());} else {$msg=get_vocab("del_user_succeed");}
    }
...
if ($room != -1) {
    ...
    if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
    {
        ...
        if ($is_admin=='yes') {
            ...
            echo "<b>";
            echo "$row[1] $row[2]</b> | <a href='admin_right.php?action=del_admin_all&amp;login_admin=".urlencode($row[0])."&amp;id_area=$id_area'>".get_vocab("delete")."</a><br />";
        }
    }
    if ($exist_admin=='no') {
