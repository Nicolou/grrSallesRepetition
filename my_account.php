...
/**
 * Chargement des fichiers de configuration et de définition des variables
 */
 ...
 */
 ...
 if ($valid == 'yes')
 {
   ...
   $sql = "SELECT email,source,nom,prenom,couleur
   ...
   $reg_couleur = isset($_POST['reg_couleur']) ? $_POST['reg_couleur'] : $user_couleur;
   $champ_manquant= 'n';
   ...
   if (($user_email != $reg_email) or ($user_nom != $reg_nom) or ($user_prenom != $reg_prenom) or ($user_couleur != $reg_couleur)) 
   {
     ...
     $sql.="couleur = '" . protect_data_sql($reg_couleur)."'";
 ...
 // Si valid='yes', la page a été invoquée pour validation de son formulaire
 // Si valid='reset' la page a été invoqué pour remettre à blanc les champs du formulaire
 ...
 $sql="SELECT nom,prenom,statut,email,default_site,default_area,default_room,default_style,default_list_type,default_language,source,couleur
       FROM ".TABLE_PREFIX."_utilisateurs
       WHERE login='".getUserName()."'";
 $res = grr_sql_query($sql);
 if ($res)
