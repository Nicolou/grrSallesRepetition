...
case "envoi":
  $destinataire = "";
  if ($_POST["type_cible"]=="identifiant:non") {
      if ($_POST["cible"]=="contact_administrateur") {
        $destinataire = getSettingValue("webmaster_email");
      } else if ($_POST["cible"]=="contact_support") {
        $destinataire = getSettingValue("technical_support_email");
      }
  } else {
    // Il s'agit d'un utilisateur de la base
        $destinataire = grr_sql_query1("SELECT email FROM ".TABLE_PREFIX."_utilisateurs WHERE login = '".$_POST["cible"]."'");
        if ($destinataire==-1)
            $destinataire="";
  }
  if ($destinataire=="") {
...
default://formulaire d'envoi
  ...
  if (isset($_GET["type_cible"]))
	    echo "<input type=\"hidden\" name=\"type_cible\" value=\"".$_GET["type_cible"]."\" />\n";
	echo "<textarea name=\"message\" cols=\"50\" rows=\"5\">Contenu du message : </textarea><br />";
