 ...
 * @package   root
 * @version   $Id: month.php,v 1.17 2009-09-29 18:02:57 grr Exp $
 * @filesource
 *
...
for ($cday = 1; $cday <= $days_in_month; $cday++)
{
    ...
    if (est_hors_reservation(mktime(0,0,0,$month,$cday,$year))) {
        ...
        if (isset($d[$cday]["id"][0])) {
            ...
            for ($i = 0; $i < $n; $i++) {
                ...
                tdcell(getUserColorNumber($d[$cday]["id"][$i]));
                ...
                if ($acces_fiche_reservation)
							{
                    	$nom_reservation = $d[$cday]["who1"][$i];
	                    echo "<a title=\"".htmlspecialchars($d[$cday]["who"][$i])."\" href=\"view_entry.php?id=" . $d[$cday]["id"][$i]
   	                 . "&amp;day=$cday&amp;month=$month&amp;year=$year&amp;page=month\">"
      	              . $nom_reservation . "</a>"
         	           . "<a href=mailto:". affiche_nom_prenom_email($nom_reservation,"","onlymail") . '>&nbsp;&nbsp;<img src="images/stock_mail-unread.png" alt="mailTo" title="Envoyer un Email" width="24" height="24" class="image" /></a>';
            		}          
                else
                    echo $d[$cday]["who1"][$i];
    ...
    and ($UserRoomMaxBooking != 0)
      ...
      echo "<div class=\"empty_cell\">";
      if ($enable_periods == 'y')
				if   ($date_booking < $date_now) echo "<a href=\"edit_entry.php?room=".$room."&amp;period=&amp;year=$year&amp;month=$month&amp;day=$cday&amp;page=month\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"img_grr/new.gif\" alt=\"".get_vocab("add")."\" class=\"".$class_image."\"  /></a>"; 				
				else echo "<a href=\"edit_entry.php?room=".$room."&amp;period=&amp;year=$year&amp;month=$month&amp;day=$cday&amp;page=month\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"img_grr/new.png\" alt=\"".get_vocab("add")."\" class=\"".$class_image."\"  /></a>";
			else
				if   ($date_booking < $date_now) echo "<a href=\"edit_entry.php?room=".$room."&amp;period=&amp;year=$year&amp;month=$month&amp;day=$cday&amp;page=month\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"img_grr/new.gif\" alt=\"".get_vocab("add")."\" class=\"".$class_image."\"  /></a>";				
				else echo "<a href=\"edit_entry.php?room=".$room."&amp;hour=$hour&amp;minute=0&amp;year=$year&amp;month=$month&amp;day=$cday&amp;page=month\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"img_grr/new.png\" alt=\"".get_vocab("add")."\" class=\"".$class_image."\" /></a>";
      echo "</div>";
    } else {
			echo "&nbsp;";
		}
