 ...
 * @package   root
 * @version   $Id: week.php,v 1.16 2009-06-04 15:30:17 grr Exp $
 * @filesource
 *
...
for ($slot = $first_slot; $slot <= $last_slot; $slot++)
{
    ...
    for ($weekday = 0; $weekday < 7; $weekday++)
    {
        ...
        {          
            ...
            and $_GET['pview'] != 1) {
					 if ($debug_flag) echo '<p>OK</p>';                
                if ($enable_periods=='y') {
                    echo "<a href=\"edit_entry.php?room=$room" . "&amp;period=$time_t_stripped&amp;year=$wyear&amp;month=$wmonth";
                    if   ($date_booking < $date_now) echo "&amp;day=$wday&amp;page=week\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"img_grr/new.gif\" alt=\"".get_vocab("add")."\" width=\"10\" height=\"10\"  class=\"".$class_image."\"  />";     
                    else echo "&amp;day=$wday&amp;page=week\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"img_grr/new.png\" alt=\"".get_vocab("add")."\" width=\"16\" height=\"16\"  class=\"".$class_image."\"  />";
                    echo "</a>";
                } else {
                    echo "<a href=\"edit_entry.php?room=$room" . "&amp;hour=$hour&amp;minute=$minute&amp;year=$wyear&amp;month=$wmonth";
						  if   ($date_booking < $date_now) echo "&amp;day=$wday&amp;page=week\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"img_grr/new.gif\" alt=\"".get_vocab("add")."\" width=\"10\" height=\"10\"  class=\"".$class_image."\"  />";     
                    else echo "&amp;day=$wday&amp;page=week\" title=\"".get_vocab("cliquez_pour_effectuer_une_reservation")."\"><img src=\"img_grr/new.png\" alt=\"".get_vocab("add")."\" width=\"16\" height=\"16\"  class=\"".$class_image."\"  />";
                    echo "</a>";
                }
            } else {
					// si creneau libre et hors du temps de resa alors si administrateur/gerant du site, on autorise une saisie 
					// sinon rien					
					echo "&nbsp;";                
					if ($debug_flag) echo '<p>rien</p>';
            }
          ...
          if (est_hors_reservation(mktime(0,0,0,$wmonth,$wday,$wyear)))
            ...
            $color = getUserColorNumber($d[$weekday][$slot-$decale_slot*$nb_case]["id"]);
            if ($color=='')  $color = $saveColor[$weekday];
            $saveColor[$weekday] = $color;
				tdcell($color);
...
if ($debug_flag) echo '<p>0</p>';                
                if (($this_statut_room == "1") or
                (($this_statut_room == "0") and (authGetUserLevel(getUserName(),$room) > 2) ))

                {
if ($debug_flag) echo '<p>1</p>';                 
                    if ($acces_fiche_reservation)
                    	{
                    	$nom_reservation = $d[$weekday][$slot-$decale_slot*$nb_case]["data"];
                      ...
                      . $nom_reservation . "</a>";
                      echo "<a href=mailto:". affiche_nom_prenom_email($nom_reservation,"","onlymail") . '>&nbsp;&nbsp;<img src="images/stock_mail-unread.png" alt="mailTo" title="Envoyer un Email" width="24" height="24" class="image" /></a>';
                      //echo "<a href=mailto:". affiche_nom_prenom_email($nom_reservation,"","onlymail") . '>&nbsp;&nbsp;@</a>';
                      }
                    else
