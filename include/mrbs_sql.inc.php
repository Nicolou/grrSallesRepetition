 ...
 * @package   include
 * @version   $Id: mrbs_sql.inc.php,v 1.14 2009-09-29 18:02:57 grr Exp $
 * @filesource
 *
...
//      $end_string = "</".$fieldslist[$field]["id"].">";    //tructruc
        ...
        while(($traitement1!==false) or ($traitement2!==false)) {
           ...
           if ($traitement1!=false) {
             $chaine1 = strstr ( $chaine  , $begin_string  ); // retourne la sous-chaîne de $chaine, allant de la première occurrence de $begin_string jusqu'à la fin de la chaîne.
             if ($chaine1!==false) { // on a trouvé une occurence de $begin_string
               $balise_ouvrante='y'; // on sait qu'il y a au moins une balise ouvrante
               $chaine = substr($chaine1,$l1,strlen($chaine1)-$l1); // on retourne la chaine en ayant éliminé le début de chaine correspondant à $begin_string
