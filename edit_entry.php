 ...
 * @package   root
 * @version   $Id: edit_entry.php,v 1.15 2009-09-29 18:02:56 grr Exp $
 * @filesource
 *
...
// verification du depasement de forfait pour l'utilisateur l'ambda ( lambda = 2, superviseur=3 admin= 6)
// lors de l'ajout d'une resa
if ($compt == 1 and  authGetUserLevel(getUserName(),$room) < 3 ) {
	if (verif_depassement_forfait(getUserName())) {
		exit();
		}
}
...
if (isset($id))
{
    ...
    $beneficiaire   = $row[1];
    //$breve_description = $beneficiaire;
...
<?php
    if (getSettingValue("remplissage_description_breve")=='2') {
?>
	  name.value=document.forms["main"].beneficiaire.options[document.forms["main"].beneficiaire.options.selectedIndex].text;
<?php
    }
?>
    } else {
...
<?php if (getSettingValue("remplissage_description_breve")=='1') { ?>
  ...
  //verifie que l on ne depasse pas 24 heures de reservation
	var minutesDay = 0;	
	var minutesDay = parseFloat(document.forms["main"].hour.value) * 60 + parseFloat(document.forms["main"].minute.value);
	if (document.forms["main"].dur_units.value == "hours") minutesDay = minutesDay + parseFloat(document.forms["main"].duration.value)*60
	else minutesDay = minutesDay + parseFloat(document.forms["main"].duration.value) ;
	if (minutesDay > 24*60) {
	alert("votre crénau horaire dépasse minuit, veuillez modifier la durée de la réservation");
    return false;
  }
...
if(((authGetUserLevel(getUserName(),-1,"room") >= $qui_peut_reserver_pour) or (authGetUserLevel(getUserName(),$area,"area") >= $qui_peut_reserver_pour))
 and (($id == 0) or (($id!=0) and ($create_by==getUserName()) )))
 {
    ...
    echo "<tr><td class=\"CL\"><select size=\"1\" name=\"beneficiaire\" onchange=\"setdefault('beneficiaire_default','');check_4();\">\n";
    ...
    if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++) {
        ...
        if ((!$cookie and strtolower($beneficiaire) == strtolower($row[0])) or ($cookie and $cookie==$row[0]))  echo " selected=\"selected\" ";
...
echo "<tr><td class=\"CL\"><input id=\"name\" name=\"name\" type=\"hidden\" size=\"80\" value=\"$C\" /></td></tr>";
